...
namespace SDGA
{
    internal static partial class AnimationUtils
    {
        ...
        public static Func<float, float> FLinear = (x) => x;
        public static Func<float, float> FEaseOutExpo = (x) => x == 1.0f ? 1 : 1 - Mathf.Pow(2.0f, -10.0f * x);
        public static Func<float, float> FEaseOutQuart = (x) => 1.0f - Mathf.Pow(1.0f - x, 4.0f);
        public static Func<float, float> FEaseInOutCubic = (x) => x < 0.5f ? 4.0f * x * x * x : 1.0f - Mathf.Pow(-2.0f * x + 2.0f, 3.0f) * 0.5f;
        public static Func<float, float> FEaseOutElastic = (x) =>
        {
